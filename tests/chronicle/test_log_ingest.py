...
def test_create_forwarder(chronicle_client, mock_forwarder_response):
    ...
    with patch.object(
        ...
        "request",
        return_value=mock_forwarder_response,
    ):
        ...
        call_args = chronicle_client.session.request.call_args
...
def test_create_forwarder_error(chronicle_client):
    ...
    with patch.object(
        chronicle_client.session, "request", return_value=error_response
    ):
...
def test_get_or_create_forwarder_new(
    ...
    with patch.object(
        chronicle_client.session,
        "request",
        side_effect=[empty_response, mock_forwarder_response],
    ):
        result = get_or_create_forwarder(
            client=chronicle_client, display_name="Wrapper-SDK-Forwarder"
        )
...
def test_ingest_log_basic(
    ...
    with (
        patch.object(
            chronicle_client.session,
            "request",
            side_effect=[mock_forwarders_list_response, mock_ingest_response],
        ),
        patch(
...
def test_ingest_log_invalid_timestamps(chronicle_client):
    ...
    with (
        pytest.raises(
            ValueError,
            match="Collection time must be same or after log entry time",
        ),
        patch(
            "secops.chronicle.log_ingest.is_valid_log_type", return_value=True
        ),
...
def test_ingest_udm_single_event(
    ...
    with patch.object(
        chronicle_client.session, "request", return_value=mock_udm_response
    ):
...
def test_ingest_log_batch(
    ...
    test_logs = [
        {"test": "log1", "message": "First message"},
        {"test": "log2", "message": "Second message"},
        {"test": "log3", "message": "Third message"},
    ]
    ...
    with (
        ...
        result = ingest_log(
            client=chronicle_client,
            log_type="OKTA",
            log_message=[json.dumps(log) for log in test_logs],
        )
