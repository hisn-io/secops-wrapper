...
def _paginated_request(
    ...
    *,
    ...
    while True:
        ...
        # If caller provided page_size, return single page response
        if page_size is not None:
            # Return original response with items
            return data
    ...
    # Return full response with all accumulated items
    last_response[items_key] = results
    # Remove nextPageToken for auto-paginated responses
...
def search_curated_detections(
    ...
    list_basis: Union[ListBasis, str] = ListBasis.LIST_BASIS_UNSPECIFIED,
    ...
    This method searches for detections created by curated rules across
    all versions of the specified rule. Results can be filtered by time
    range, alert state, and sorted by detection or creation time.

    Args:
        client: ChronicleClient instance
        rule_id: Required. Unique ID of the curated rule to search
            detections for. Examples:
                - "ur_ffac5fa0-5b0b-463e-9f92-2443f8f1b6fd"
                - "ur_ttp_GCP_MassSecretDeletion"
        start_time: Optional. The time to start searching detections
        ...
        list_basis: Basis for determining whether to apply time
            filters. Can be ListBasis enum or string. Valid values:
                - ListBasis.LIST_BASIS_UNSPECIFIED (default)
                - ListBasis.DETECTION_TIME
                - ListBasis.CREATED_TIME
        alert_state: Optional. Filter detections by alert state.
            Can be AlertState enum or string. Valid values:
                - AlertState.UNSPECIFIED (returns all)
                - AlertState.NOT_ALERTING
                - AlertState.ALERTING
        page_size: Optional. Maximum number of detections to return.
            If unspecified, at most 100 detections will be returned.
            Maximum value is 1000. If provided, only returns that page.
        page_token: Optional. Token received from a previous call to
            retrieve the next page of results.
        max_resp_size_bytes: Optional. Maximum size of response in
            bytes. If set to 0 or omitted, no limit is enforced.
        include_nested_detections: Optional. If True, include one level
    ...
    Example:
        ...
        >>> result = client.search_curated_detections(
        ...
        ... )
        >>> print(f"Found {len(result['curatedDetections'])} detections")
    """
    # Build extra parameters for the request
    extra_params = {
        "ruleId": rule_id,
    }
    ...
    if alert_state:
        ...
        # Validate the value
        valid_states = [s.value for s in AlertState]
        if alert_state_value not in valid_states:
            raise ValueError(
                f"alert_state must be one of {valid_states}, "
                f"got {alert_state_value}"
            )
        extra_params["alertState"] = alert_state_value

    # Convert enum to string value if needed
    list_basis_value = (
        list_basis.value if isinstance(list_basis, ListBasis) else list_basis
    )
    # Validate the value
    valid_basis = [b.value for b in ListBasis]
    if list_basis_value not in valid_basis:
        raise ValueError(
            f"list_basis must be one of {valid_basis}, "
            f"got {list_basis_value}"
        )
    extra_params["listBasis"] = list_basis_value
    ...
    try:
        ...
        # _paginated_request now always returns dict with full response
        return result
